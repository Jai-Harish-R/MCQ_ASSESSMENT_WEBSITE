...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, role, full_name, phone_no, profession, institution_name, department, class_name, year, designation)
  VALUES (
    new.id,
    new.email,
...
-- Add short_id for simple identity
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS short_id BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE;
