                ...
                <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '32px', border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                  ...
                  {selectedTest ? (
                    <>
                      ...
                      {/* Leaderboard Table */}
                      <div style={{ width: '100%' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '60px 1.5fr 1fr 1fr 1fr 1fr 1fr', padding: '16px', borderBottom: '1px solid #e2e8f0', fontSize: '12px', fontWeight: '700', color: '#64748b', alignItems: 'center' }}>
                        ...
                        {leaderboardAttempts.length === 0 ? (
                          ...
                          <>
                            {leaderboardAttempts.slice((leaderboardPage - 1) * itemsPerPage, leaderboardPage * itemsPerPage).map((st, i) => {
                              ...
                              return (
                                <div key={st.id} title={`Student Marks: ${st.score} / ${st.total_questions}`} style={{ display: 'grid', cursor: 'help', gridTemplateColumns: '60px 1.5fr 1fr 1fr 1fr 1fr 1fr', padding: '16px', borderBottom: '1px solid #f1f5f9', alignItems: 'center', transition: 'background-color 0.2s' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ position: 'relative' }}>
                          ...
                          <Select
                            value={tests.map(t => ({ value: t.id, label: `${t.title} ${t.short_id ? `- ${t.short_id}` : ''}` })).find(o => o.value === selectedLeaderboardTestId) || null}
                            onChange={(option: any) => setSelectedLeaderboardTestId(option ? option.value : '')}
                            options={tests.map(t => ({ value: t.id, label: `${t.title} ${t.short_id ? `- ${t.short_id}` : ''}` }))}
